...
**Goal:** Initialize the BWP-Notebook-v2 repository from Plane CE `v1.4.2`, map its architecture with attribution to IT Leon, configure a GitHub Actions CI/CD pipeline, and deploy a collision-free baseline stack on remote server `192.168.3.168` on port 18080.
...
- All new documentation, config files, customized scripts, and commit messages must include attribution: `Code & Architecture by IT Leon`.
...
```bash
git add .
git commit -m "chore(baseline): initialize BWP-Notebook-v2 from Plane CE v1.4.2 - Code by IT Leon"
```
...
- Attribution: `Code & Architecture by IT Leon`
...
```bash
git add MAPPING.md
git commit -m "docs: add architecture and domain mapping document - Code by IT Leon"
```
...
- Attribution header: `Code & Architecture by IT Leon`.
...
git commit -m "ci: add GitHub Actions workflow for GHCR image publishing - Code by IT Leon"
```
...
git commit -m "feat(deploy): add production docker-compose with port 18080 collision avoidance - Code by IT Leon"
```
