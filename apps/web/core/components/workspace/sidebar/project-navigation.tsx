...
export const ProjectNavigation = observer(function ProjectNavigation(props: TProjectItemsProps) {
  ...
  const baseNavigation = useCallback(
    (wSlug: string, pId: string): TNavigationItem[] => [
      {
        i18n_key: "sidebar.work_items",
        key: "work_items",
        name: "Work items",
        href: `/${wSlug}/projects/${pId}/issues`,
        icon: WorkItemsIcon,
      ...
      },
      {
        ...
        key: "intake",
        name: "Intake",
        href: `/${wSlug}/projects/${pId}/intake`,
        icon: IntakeIcon,
        access: [EUserPermissions.ADMIN, EUserPermissions.MEMBER, EUserPermissions.GUEST],
  ...
  const navigationItemsMemo = useMemo(() => {
    const navigationItems = (wSlug: string, pId: string): TNavigationItem[] => {
      const navItems = baseNavigation(wSlug, pId);

      if (additionalNavigationItems) {
        navItems.push(...additionalNavigationItems(wSlug, pId));
      }
